...
class Resource2Viewer{
	...
	public function show($cell, $params){
		...
		$val='<div class="res_value_link"><a href="?action=2315'.$link.'">'.$cell->value.'</a></div>';
		return '<div style="margin-bottom: 15px" onclick="load_data(2315, '.$this->params[5050].', '.$cell->id.')">'.$val.$link1.$link2.$link3.'</div>';
	}
}